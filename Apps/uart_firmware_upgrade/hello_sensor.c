...
/** @file
*
* BLE Vendor Specific Device with Peripheral UART Firmware Upgrade
*
...
*  - Firmware upgrade over Peripheral UART
*
...
* 6. Use WsUartFu application to try UART upgrade
*
*/
...
static void hello_sensor_puart_interrupt_callback(void* unused);
...
void hello_sensor_create(void)
{
    ...
    blecm_setTxPowerInADV(0);
   	...
   	if (!ws_upgrade_uart_init(&hello_sensor_puart_cfg, hello_sensor_puart_interrupt_callback))
    {
...
void hello_sensor_puart_interrupt_callback(void* unused)
{
    // ble_trace0("\rhello_sensor_puart_interrupt_callback");
    ws_upgrade_uart_interrupt_callback(unused);
}
